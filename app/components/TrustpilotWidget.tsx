"use client";

import { useEffect, useRef } from "react";
import Script from "next/script";

/**
 * Widget Trustpilot carousel officiel
 * - Synchronisation automatique avec Trustpilot
 * - Avis 4 et 5 étoiles uniquement
 * - Langue française
 * - Thème clair
 */
export default function TrustpilotWidget() {
  const trustboxRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    // Charger le widget Trustpilot quand le composant est monté
    if (window.Trustpilot && trustboxRef.current) {
      window.Trustpilot.loadFromElement(trustboxRef.current, true);
    }
  }, []);

  return (
    <>
      {/* Script Trustpilot - chargé une seule fois */}
      <Script
        src="https://widget.trustpilot.com/bootstrap/v5/tp.widget.bootstrap.min.js"
        strategy="lazyOnload"
        onLoad={() => {
          // Charger le widget après que le script soit chargé
          if (trustboxRef.current && window.Trustpilot) {
            window.Trustpilot.loadFromElement(trustboxRef.current, true);
          }
        }}
      />

      <section className="relative w-full bg-gradient-to-b from-white to-gray-50 py-16 md:py-24">
        <div className="mx-auto w-full max-w-7xl px-6 md:px-12">
          {/* Header */}
          <div className="mb-12 md:mb-16 text-center">
            <p className="text-xs tracking-[0.25em] uppercase text-[#444684] mb-4 font-semibold">
              AVIS VÉRIFIÉS TRUSTPILOT
            </p>
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-semibold leading-tight">
              Ce que nos clients
              <br className="hidden md:block" />
              <span className="text-[#444684]"> disent de nous</span>
            </h2>
            <p className="mx-auto mt-5 max-w-3xl text-base md:text-lg text-neutral-700 leading-relaxed">
              Découvrez les avis authentiques de nos clients sur Trustpilot.
              Tous les avis sont vérifiés et collectés de manière indépendante.
            </p>
          </div>

          {/* Widget Trustpilot TrustBox - Carousel */}
          <div
            ref={trustboxRef}
            className="trustpilot-widget"
            data-locale="fr-FR"
            data-template-id="53aa8912dec7e10d38f59f36"
            data-businessunit-id="675c07a7f40e16c45da82764"
            data-style-height="350px"
            data-style-width="100%"
            data-theme="light"
            data-stars="4,5"
            data-review-languages="fr"
            data-font-family="inherit"
            data-text-color="#0b0b0c"
          >
            <a
              href="https://fr.trustpilot.com/review/tanse.fr"
              target="_blank"
              rel="noopener noreferrer"
              className="text-[#444684] hover:underline text-lg font-medium"
            >
              Voir nos avis sur Trustpilot
            </a>
          </div>

          {/* Lien vers Trustpilot */}
          <div className="mt-10 text-center">
            <a
              href="https://fr.trustpilot.com/review/tanse.fr"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 text-sm text-neutral-600 hover:text-[#444684] transition-colors font-medium"
            >
              <span>Voir tous nos avis sur Trustpilot</span>
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
              </svg>
            </a>
          </div>
        </div>
      </section>
    </>
  );
}

// Déclaration TypeScript pour le widget Trustpilot
declare global {
  interface Window {
    Trustpilot?: {
      loadFromElement: (element: HTMLElement | null, refresh?: boolean) => void;
    };
  }
}
